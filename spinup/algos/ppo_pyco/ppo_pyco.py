...
"""
def ppo_pyco(env_fn, actor_critic=core.mlp_actor_critic, ac_kwargs=dict(), seed=0,
        steps_per_epoch=4000, epochs=100, gamma=0.99, clip_ratio=0.2, pi_lr=3e-2,
        vf_lr=5e-2, train_pi_iters=80, train_v_iters=80, lam=0.97, max_ep_len=500,
    ...
    pi, logp, logp_pi, v, logits = actor_critic(x_ph, a_ph, policy='relational_categorical_policy', action_space = env.action_space.n)
    ...
    def update(epoch):
        ...
        pi_l_old, v_l_old, ent = sess.run([pi_loss, v_loss, approx_ent], feed_dict={logp_old_ph: buf.logp_buf,x_ph: o, a_ph: a, adv_ph: buf.adv_buf, ret_ph: buf.ret_buf})
        ...
        for i in range(train_pi_iters):
            _, kl = sess.run([train_pi, approx_kl], feed_dict={logp_old_ph: buf.logp_buf,x_ph: o, a_ph: a, adv_ph: buf.adv_buf, ret_ph: buf.ret_buf})
            kl = mpi_avg(kl)
            if kl > 1.5 * target_kl:
        ...
        for _ in range(train_v_iters):
            sess.run(train_v, feed_dict={logp_old_ph: buf.logp_buf,x_ph: o, a_ph: a, adv_ph: buf.adv_buf, ret_ph: buf.ret_buf})

        # Log changes from update
        pi_l_new, v_l_new, kl, cf = sess.run([pi_loss, v_loss, approx_kl, clipfrac], feed_dict={logp_old_ph: buf.logp_buf,x_ph: o, a_ph: a, adv_ph: buf.adv_buf, ret_ph: buf.ret_buf})
